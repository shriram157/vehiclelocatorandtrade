	...
	return BaseController.extend("vehicleLocator.controller.VehicleTrade_CreateSingle", {        
		...
		onRequestVT: function () {
			...
			if (TradeRequest == "") {
				...
				// Changes Begin - Migration Team
				that.oDataUrl = "/odata/v2/vehicleTrade/Trade_Request?&$orderby=Trade_Id%20desc&$top=1";
				// Changes End - Migration Team
				...
				$.ajax({
					...
					//	var VIN = that.getView().getModel("TradeModel").getData().vhvin;      //changes by swetha for DMND0003618
						...
						if (Req_Proposed_ETA_FromDate != "") {
							...
							var Req_Proposed_ETA_From = null;
						}
						...
						if (Off_Proposed_ETA_FromDate != "") {
							...
							var Off_Proposed_ETA_From = null;
						}
						...
						that.oDataModel.create("/Trade_Request", oEntry, null, function (s) {
		...
		TradeComment: function (oEntry) {
			...
			// Changes Begin - Migration Team
			that.oDataUrl = "/odata/v2/vehicleTrade/Trade_Comment?$filter=Trade_Id eq '" + that.Trade_Id + "'";
			// Changes End - Migration Team
		...
		TradeStatus: function (oEntry) {
			...
			if (that.getView().byId("FromFourth").getText() != "FromPush") {
				...
				for (var i = 0; i < Tradestatus.length; i++) {
					that.oDataModel.create("/Vehicle_Desc", Tradestatus[i], null, function (s) {
						//	MessageBox.error(JSON.parse(err.response.body).error.message.value);
