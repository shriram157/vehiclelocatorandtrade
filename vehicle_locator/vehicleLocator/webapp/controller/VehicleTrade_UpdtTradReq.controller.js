	...
	return BaseController.extend("vehicleLocator.controller.VehicleTrade_UpdtTradReq", {
		...
		onRouteMatched: function (oEvent) {
			...
			if (sap.ui.getCore().getModel("SelectedSimpleFormAproveTrReq") != undefined && SelectedTrade != "VehicleTrade_updateTradeVehicle" &&
				...
				this.getView().byId("tableVrade").getBinding('items').filter([new sap.ui.model.Filter("Trade_Id", sap.ui.model.FilterOperator
					.EQ, sap.ui
		...
		VehicleDelete: function (vin) {
			...
			sap.ui.core.BusyIndicator.show(0);
			/*	that.oDataModel.create("/Vehicle", oEntry1, null, function (s) {*/
			// for (var i = 0; i < oVehicleVTN.length; i++) {
			// 	//	var VehicleUrl=	"/Vehicle(Trade_Id eq'"+Trade_Id+"' and VTN eq'"+oVehicleVTN[i]+ "')";
			var VehicleUrl = "/Vehicle(Trade_Id='" + Trade_Id + "',VTN='" + OfferedVTn + "')";
			...
			/*/Vehicle(Trade_Id='TR000001',VTN='002359')*/
		...
		TradeVehcles: function (oEntry) {
			...
			var UpdatedTreadeEntity = "/Vehicle(Trade_Id ='" + oEntry.Trade_Id + "',VTN ='" + oEntry1.VTN + "')";
		...
		TradeStatus: function (oEntry) {
			...
			var UpdatedTreadeEntity = "/Vehicle_Desc(Trade_Id ='" + oEntry.Trade_Id + "',VTN ='" + oEntry2["VTN"] +
				"',SPRAS='" + oEntry2.SPRAS + "')";
...
*/
		...
		TradeVehicleCreate: function () {
			...
			oEntry2["Trade_Id"] = VehicleTrade_Id;
			var oVehicleDetails = [];
			...
			if (that.getView().byId("otextId").getText() != "RemoveOffered") {
				...
				oEntry1["Trade_Id"] = VehicleTrade_Id;
				oVehicleDetails.push(oEntry1);
			}

			/*	oEntry1["Trade_Id"] = oEntry.Trade_Id;*/
			/*	oVehicleDetails["Trade_Id"] = oEntry.Trade_Id;
			 */
			var sLocation = window.location.host;
			var sLocation_conf = sLocation.search("webide");
			...
			for (var i = 0; i < oVehicleDetails.length; i++) {
				that.getView().getModel('TradeRequestModel').create("/Vehicle", oVehicleDetails[i], null, function (s) {
					/*	alert("ok");*/
					sap.ui.core.BusyIndicator.hide();
				}, function () {
					sap.ui.core.BusyIndicator.hide();
		...
		TradeVehicleCreateForWithOutChange: function () {
			...
			/*	that.oDataModel.create("/Vehicle", oEntry1, null, function (s) {*/
			for (var i = 0; i < oVehicleDetails.length; i++) {
				that.oDataModel.create("/Vehicle", oVehicleDetails[i], null, function (s) {
					/*	alert("ok");*/
				}, function () {
		...
		TradeVehicleCreateFromVehicle_Selection: function (offeredv) {
			...
			/*	that.oDataModel.create("/Vehicle", oEntry1, null, function (s) {*/
			// for (var i = 0; i < oVehicleDetails.length; i++) {
			that.getView().getModel('TradeRequestModel').create("/Vehicle", oEntry1, null, function (s) {
				/*	alert("ok");*/
				sap.ui.core.BusyIndicator.hide();
		...
		VehicleDesc_create: function (vin, offeredv) {
			...
			// var UpdatedTreadeEntity = "/Vehicle_Desc(Trade_Id ='" + oEntry.Trade_Id + "',VTN ='" + oEntry2["VTN"] +
			// 	"',SPRAS='" + oEntry2.SPRAS + "')";
			that.oDataModel.create("/Vehicle_Desc", oEntry2, null, function (s) {
				sap.ui.core.BusyIndicator.hide();
			...
			that.oDataModel.create("/Vehicle_Desc", oEntry1, null, function (s) {
				sap.ui.core.BusyIndicator.hide();
		...
		VehicleDesc_delete: function (vin) {
			...
			var UpdatedTreadeEntity = "/Vehicle_Desc(Trade_Id ='" + Trade_Id + "',VTN ='" + vin +
				"',SPRAS='" + "F" + "')";
			...
			var UpdatedTreadeEntity1 = "/Vehicle_Desc(Trade_Id ='" + Trade_Id + "',VTN ='" + vin +
				"',SPRAS='" + "E" + "')";
		...
		updatewioutchanges: function (vin, dealer, type) {
			...
			if (ordertype == "SO" || ordertype == "DM") {
				...
				// 	"'&$format=json";
				var SeriesUrl = that.oDataUrl + "/ZVMS_CDS_ETA_consolidate(Req_dealer='" + oDealer1 + "')/Set?$filter=zzvtn eq '" + VTN +
					"' and kunnr eq '" + dealercode +
					"'&$format=json";
